 ...
 * @date 5/17/14
 */
public class StoreCreaturesEventHandler extends EventHandler<StoreCreaturesEvent> {
    public StoreCreaturesEventHandler(EventMultiplexer eventMultiplexer) {
        super(eventMultiplexer, StoreCreaturesEvent.class);
    }

    @Override
    public void onEvent(StoreCreaturesEvent event) {
        List<CreatureEntity> creatureEntities = event.getCreatureEntities();
        // TODO: store them in db and tell ui to update
        // publishEvent(new UpdateCreaturesUIEvent(creatureEntities));
    }
}
